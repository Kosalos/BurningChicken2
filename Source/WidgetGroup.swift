...
class WidgetGroup: UIView {
    ...
    func wgAlterZoom(_ dz:Float) { delegate?.wgAlterZoom(dz) }
    ...
    func drawGraph(_ index:Int) {
        let d = data[index]
        let x:CGFloat = 5
        let rect = CGRect(x:x, y:py+GphYoff, width:GrphSZ, height:GrphSZ)
    ...
    func drawEntry(_ index:Int) {
        ...
        case .legend :
            drawText(Tab2,py+TxtYoff,.lightGray,FontSZ,data[index].str[0])
